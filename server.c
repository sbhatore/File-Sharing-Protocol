...
int socketfd, newsocketfd,rfd;
	...
	servi_addr.sin_port = htons(5006);
...
while(1)
		{
			...
			rfd = read(newsocketfd, read_buffer, 1024);
			if(rfd>0)
    ...
    if (dp != NULL)
    {
        while (ep = readdir (dp))
        {
            ...
            else
            {
                ...
                write(*newsocketfd, write_buffer, 1024);
            }
        }
        closedir (dp);
    }
...
//	puts(a);
	...
	puts(write_buffer);
	if(n=write(*socketfd,write_buffer,1024)<0)
	{
...
void peer()
{
    ...
    while(1)
    {
		...
		{
			wfd = write(socketfd,write_buffer,1024);
			if (wfd < 0) 
...
void clientIndexget(char write_buffer[],int *socketfd)
{
	//	printf("client...");
	int writefd;
	writefd = write(*socketfd,write_buffer,1024);
	if (writefd < 0)  
		put_error("ERROR writing to socket");
	char l[10]="--longlist",read_buffer[1024],r[7]="--regex",s[11]="--shortlist";
	int type=0,i;
	for(i=0;i<10;i++)
...
void clientFilehash(char write_buffer[], int *socketfd)
{
	int readfd;
	char read_buffer[1024];
	readfd = write(*socketfd,write_buffer,1024);
	if (readfd < 0)  
		put_error("ERROR writing to socket");
	int type=0,i;
	char v[8]="--verify",c[10]="--checkall";
